...
export async function addMute (guild: Guild, target: GuildMember, roleId: string, channel: TextChannel, duration: number = 30) {
  const revoke = new Date(Date.now() + duration * 60000)
...
export async function revokeMute (client: Client, guild: Guild, target: GuildMember, args: [string, string, Date]) {
  ...
  if (mute) {
    const role = guild.roles.cache.get(mute)
  ...
  const data = await queryHelper(queryString1, args)
  if (data && data.length >= 1) {
    const roleIds = JSON.parse(data[0].roles)
    ...
    const channel = guild.channels.cache.get(data[0].channel) as TextChannel
