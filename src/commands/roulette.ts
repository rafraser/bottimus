...
module.exports = {
    ...
    async execute(client: Client, message: Message, args: string[]) {
        ...
        let credits = await getArcadeCredits(message.member.id)
        if (credits < betAmount) {
            message.channel.send(`You don't have enough coins for this!`)
            return
        }

        let confirm_msg = await message.channel.send(`Are you sure you want to bet **${betAmount}** coins on ${betType}?`)
        confirm_msg.react('✅')
        const filter = (reaction: MessageReaction, user: User) => {
            return user.id === message.member.id && reaction.emoji.name === '✅'
        }

        const collector = confirm_msg.createReactionCollector(filter, { time: 35000 })
        collector.on('collect', () => {
            // Confirmation received!
            collector.stop()
            confirm_msg.edit('Get ready!')
