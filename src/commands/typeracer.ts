...
async function incrementStatScore (client: Client, userid: string, speed: number): Promise<[boolean] | [boolean, string, number]> {
  ...
  const speedResults = await queryHelper(queryTwo, [userid])
  const best = speedResults[0].speed_best
  if (speed > best) {
    await queryHelper(queryThree, [speed, new Date(), userid])
    return [true, userid, speed]
  } else {
    return [false]
  }
}
...
async function startTypeRacer (client: Client, message: Message, display: Message) {
  ...
  collector.on('end', async () => {
    const letters = list.join(' ').length
    let place = 1
    ...
    for (const result of winners) {
      ...
      const record = await incrementStatScore(client, result[0], wpm)
      if (record[0]) {
        const id = record[1]
